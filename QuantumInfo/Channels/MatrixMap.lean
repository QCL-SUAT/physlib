...
theorem kron_def [CommSemiring R] (M₁ : MatrixMap A B R) (M₂ : MatrixMap C D R) (M : Matrix (A × C) (A × C) R) :
  ...
  simp only [Matrix.stdBasis,
    ...
    Module.Basis.repr_reindex,     smul_eq_mul,
  ...
  · rw [ Finset.sum_eq_single ( ( a₁, c₁ ), ( a₂, c₂ ) ) ]
    · simp only [↓reduceIte, Pi.single_eq_same, mul_one]
      rw [ mul_comm ]
      -- v4.34 note: the three factors below are the `Pi.single`-basis evaluation (which is `1`
      -- at its own index), and the `M₂`/`M₁` entries; v4.30's `congr!` used to leave this as a
      -- matrix problem for `ext i j`, while v4.34 decomposes it into the scalar subgoals
      -- directly, so the three factors are identified by hand here.
      have hZ : (Finsupp.equivMapDomain (Equiv.sigmaEquivProd (A × C) (A × C))
            ((Pi.basis fun x => Pi.basisFun R (A × C)).repr
              (Pi.single (a₁, c₁) (Pi.single (a₂, c₂) 1)))) ((a₁, c₁), a₂, c₂) = 1 := by
        simp
      have hsingle₂ : Matrix.of (Pi.single c₁ (Pi.single c₂ (1 : R))) = Matrix.single c₁ c₂ 1 := by
        ext i' j'
        simp only [Matrix.of_apply, Matrix.single_apply, Pi.single_apply]
        by_cases h1 : i' = c₁ <;> by_cases h2 : j' = c₂ <;> simp_all [eq_comm]
      have hsingle₁ : Matrix.of (Pi.single a₁ (Pi.single a₂ (1 : R))) = Matrix.single a₁ a₂ 1 := by
        ext i' j'
        simp only [Matrix.of_apply, Matrix.single_apply, Pi.single_apply]
        by_cases h1 : i' = a₁ <;> by_cases h2 : j' = a₂ <;> simp_all [eq_comm]
      have hM2 : (Finsupp.equivMapDomain (Equiv.sigmaEquivProd D D)
            ((Pi.basis fun x => Pi.basisFun R D).repr
              (Matrix.of.symm (M₂ (Matrix.of (Pi.single c₁ (Pi.single c₂ 1))))))) (d₁, d₂)
          = M₂ (Matrix.single c₁ c₂ 1) d₁ d₂ := by
        simp [Matrix.of_symm_apply, hsingle₂]
      have hM1 : (Finsupp.equivMapDomain (Equiv.sigmaEquivProd B B)
            ((Pi.basis fun x => Pi.basisFun R B).repr
              (Matrix.of.symm (M₁ (Matrix.of (Pi.single a₁ (Pi.single a₂ 1))))))) (b₁, b₂)
          = M₁ (Matrix.single a₁ a₂ 1) b₁ b₂ := by
        simp [Matrix.of_symm_apply, hsingle₁]
      rw [hZ, hM2, hM1, one_mul, mul_comm]
    · intro b _ hne
      have hZ0 : (Finsupp.equivMapDomain (Equiv.sigmaEquivProd (A × C) (A × C))
            ((Pi.basis fun x => Pi.basisFun R (A × C)).repr
              (Pi.single (a₁, c₁) (Pi.single (a₂, c₂) 1)))) b = 0 := by
        simp only [Finsupp.equivMapDomain_apply,  Pi.basis_repr,
          Pi.basisFun_repr, Pi.single_apply]
        by_cases h1 : b.1 = (a₁, c₁)
        · by_cases h2 : b.2 = (a₂, c₂)
