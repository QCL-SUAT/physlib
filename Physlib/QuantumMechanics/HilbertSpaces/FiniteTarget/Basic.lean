...
/-- `equivEuclidean` bundled as an additive equivalence (v4.34 moved `Equiv.module`/
  `Equiv.linearEquiv` to `AddEquiv`, so the transported structure needs one). -/
noncomputable def equivEuclideanAddEquiv :
    FiniteHilbertSpace d ≃+ EuclideanSpace ℂ d where
  toFun := equivEuclidean
  invFun := equivEuclidean.symm
  left_inv := equivEuclidean.left_inv
  right_inv := equivEuclidean.right_inv
  map_add' _ _ := rfl

noncomputable instance : Module ℂ (FiniteHilbertSpace d) := equivEuclideanAddEquiv.module ℂ
...
noncomputable def linearEquivEuclidean : FiniteHilbertSpace d ≃ₗ[ℂ] EuclideanSpace ℂ d :=
  equivEuclideanAddEquiv.linearEquiv ℂ
