...
/-- `decomposeEquiv` bundled as an additive equivalence (v4.34 moved `Equiv.module`/
  `Equiv.linearEquiv` to `AddEquiv`, so the transported structure needs one). -/
def decomposeEquivAddEquiv : Dirac ≃+ LeftHandedWeyl × DualRightHandedWeyl where
  toFun := decomposeEquiv
  invFun := decomposeEquiv.symm
  left_inv := decomposeEquiv.left_inv
  right_inv := decomposeEquiv.right_inv
  map_add' _ _ := rfl

instance : Module ℂ Dirac := decomposeEquivAddEquiv.module ℂ
