...
def toFin2ℂFunAddEquiv : RightHandedWeyl ≃+ (Fin 2 → ℂ) where
  toFun := toFin2ℂFun
...
/-- The instance of `Module` on `RightHandedWeyl` defined via its equivalence
  with `Fin 2 → ℂ`. -/
instance : Module ℂ RightHandedWeyl := toFin2ℂFunAddEquiv.module ℂ
...
def basis : Basis (Fin 2) ℂ RightHandedWeyl := Basis.ofEquivFun
  (RightHandedWeyl.toFin2ℂFunAddEquiv.linearEquiv ℂ)
