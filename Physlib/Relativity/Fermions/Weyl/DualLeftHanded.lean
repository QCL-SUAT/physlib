...
def toFin2ℂFunAddEquiv : DualLeftHandedWeyl ≃+ (Fin 2 → ℂ) where
  toFun := toFin2ℂFun
...
/-- The instance of `Module` on `DualLeftHandedWeyl` defined via its equivalence
  with `Fin 2 → ℂ`. -/
instance : Module ℂ DualLeftHandedWeyl := toFin2ℂFunAddEquiv.module ℂ
...
def basis : Basis (Fin 2) ℂ DualLeftHandedWeyl := Basis.ofEquivFun
  (DualLeftHandedWeyl.toFin2ℂFunAddEquiv.linearEquiv ℂ)
