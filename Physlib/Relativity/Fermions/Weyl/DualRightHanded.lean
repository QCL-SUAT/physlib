...
/-- `toFin2ℂFun` bundled as an additive equivalence (v4.34 moved `Equiv.module`/
  `Equiv.linearEquiv` to `AddEquiv`, so the transported structure needs one). -/
def toFin2ℂFunAddEquiv : DualRightHandedWeyl ≃+ (Fin 2 → ℂ) where
  toFun := toFin2ℂFun
  invFun := toFin2ℂFun.symm
  left_inv := toFin2ℂFun.left_inv
  right_inv := toFin2ℂFun.right_inv
  map_add' _ _ := rfl

/-- The instance of `Module` on `DualRightHandedWeyl` defined via its equivalence
  with `Fin 2 → ℂ`. -/
instance : Module ℂ DualRightHandedWeyl := toFin2ℂFunAddEquiv.module ℂ
...
def basis : Basis (Fin 2) ℂ DualRightHandedWeyl := Basis.ofEquivFun
  (DualRightHandedWeyl.toFin2ℂFunAddEquiv.linearEquiv ℂ)
