...
def toFin2ℂFunAddEquiv : LeftHandedWeyl ≃+ (Fin 2 → ℂ) where
  toFun := toFin2ℂFun
...
/-- The instance of `Module` on `LeftHandedWeyl` defined via its equivalence
  with `Fin 2 → ℂ`. -/
instance : Module ℂ LeftHandedWeyl := toFin2ℂFunAddEquiv.module ℂ
...
def basis : Basis (Fin 2) ℂ LeftHandedWeyl := Basis.ofEquivFun
  (LeftHandedWeyl.toFin2ℂFunAddEquiv.linearEquiv ℂ)
