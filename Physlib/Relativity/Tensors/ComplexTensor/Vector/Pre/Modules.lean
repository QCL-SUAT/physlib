...
/-- `toFin13ℂFun` bundled as an additive equivalence (v4.34 moved `Equiv.module`/
  `Equiv.linearEquiv` to `AddEquiv`, so the transported structure needs one). -/
def toFin13ℂFunAddEquiv : ContrℂModule ≃+ (Fin 1 ⊕ Fin 3 → ℂ) where
  toFun := toFin13ℂFun
  invFun := toFin13ℂFun.symm
  left_inv := toFin13ℂFun.left_inv
  right_inv := toFin13ℂFun.right_inv
  map_add' _ _ := rfl

/-- The instance of `Module` on `ContrℂModule` defined via its equivalence
  with `Fin 1 ⊕ Fin 3 → ℂ`. -/
instance : Module ℂ ContrℂModule := toFin13ℂFunAddEquiv.module ℂ
...
def toFin13ℂEquiv : ContrℂModule ≃ₗ[ℂ] (Fin 1 ⊕ Fin 3 → ℂ) :=
  toFin13ℂFunAddEquiv.linearEquiv ℂ
...
/-- `toFin13ℂFun` bundled as an additive equivalence (v4.34 moved `Equiv.module`/
  `Equiv.linearEquiv` to `AddEquiv`, so the transported structure needs one). -/
def toFin13ℂFunAddEquiv : CoℂModule ≃+ (Fin 1 ⊕ Fin 3 → ℂ) where
  toFun := toFin13ℂFun
  invFun := toFin13ℂFun.symm
  left_inv := toFin13ℂFun.left_inv
  right_inv := toFin13ℂFun.right_inv
...
/-- The instance of `Module` on `CoℂModule` defined via its equivalence
  with `Fin 1 ⊕ Fin 3 → ℂ`. -/
instance : Module ℂ CoℂModule := toFin13ℂFunAddEquiv.module ℂ
