...
/-- The additive equivalence between `ContrMod` and `Fin 1 ⊕ Fin d → ℝ`,
  bundling `toFin1dℝFun` with the `AddCommGroup` structure defined above. -/
def toFin1dℝAddEquiv : ContrMod d ≃+ (Fin 1 ⊕ Fin d → ℝ) where
  toFun := toFin1dℝFun
  invFun := toFin1dℝFun.symm
  left_inv := toFin1dℝFun.left_inv
  right_inv := toFin1dℝFun.right_inv
  map_add' _ _ := rfl

/-- The instance of `Module` on `ContrMod` defined via its equivalence
  with `Fin 1 ⊕ Fin d → ℝ`. -/
instance : Module ℝ (ContrMod d) := toFin1dℝAddEquiv.module ℝ
...
def toFin1dℝEquiv : ContrMod d ≃ₗ[ℝ] (Fin 1 ⊕ Fin d → ℝ) :=
  toFin1dℝAddEquiv.linearEquiv ℝ
...
/-- `toFin1dℝFun` bundled as an additive equivalence (v4.34 moved `Equiv.module`/
  `Equiv.linearEquiv` to `AddEquiv`, so the transported structure needs one). -/
def toFin1dℝAddEquiv : CoMod d ≃+ (Fin 1 ⊕ Fin d → ℝ) where
  toFun := toFin1dℝFun
  invFun := toFin1dℝFun.symm
  left_inv := toFin1dℝFun.left_inv
  right_inv := toFin1dℝFun.right_inv
...
/-- The instance of `Module` on `CoℝModule` defined via its equivalence
  with `Fin 1 ⊕ Fin d → ℝ`. -/
instance : Module ℝ (CoMod d) := toFin1dℝAddEquiv.module ℝ
