...
/-- `componentEquiv` bundled as an additive equivalence (v4.34 moved `Equiv.module`/
  `Equiv.linearEquiv` to `AddEquiv`, so the transported structure needs one). -/
def componentAddEquiv : frame.Vector ≃+ (Fin d → ℝ) where
  toFun := componentEquiv
  invFun := componentEquiv.symm
  left_inv := componentEquiv.left_inv
  right_inv := componentEquiv.right_inv
  map_add' _ _ := rfl

instance : Module ℝ frame.Vector := componentAddEquiv.module ℝ
...
def componentLinearEquiv : frame.Vector ≃ₗ[ℝ] (Fin d → ℝ) :=
  componentAddEquiv.linearEquiv ℝ
