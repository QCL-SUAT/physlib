...
/-- v4.34: mathlib's `LinearPMap.ker` is the ambient-domain kernel
`f.toFun.ker.map f.domain.subtype`, while the lemmas below state hypotheses in the
older submodule-of-the-domain form `f.toFun.ker = ⊥`. This bridges the two. -/
lemma ker_eq_bot_of_toFun_ker_eq_bot (h : f.toFun.ker = ⊥) : f.ker = ⊥ := by
  rw [LinearPMap.ker, h]
  exact Submodule.map_bot _

variable (h_ker : f.toFun.ker = ⊥)
include h_ker

lemma inverse_ker : f.inverse.toFun.ker = ⊥ := by
  refine LinearMap.ker_eq_bot'.mpr fun ⟨y, hy⟩ hy' ↦ ?_
  obtain ⟨x, hx⟩ := inverse_domain (f := f) ▸ hy
  simp_all [inverse_apply_eq (x := x) (y := ⟨y, hy⟩) (ker_eq_bot_of_toFun_ker_eq_bot h_ker) hx]

lemma inverse_inverse : f.inverse.inverse = f := by
  ext x hx hx'
  · rw [inverse_domain, inverse_range (ker_eq_bot_of_toFun_ker_eq_bot h_ker)]
  · refine inverse_apply_eq (y := ⟨x, hx⟩) (x := ⟨f ⟨x, hx'⟩, by simp [inverse_domain]⟩) ?_ ?_
    · exact ker_eq_bot_of_toFun_ker_eq_bot (inverse_ker h_ker)
    · exact inverse_apply_eq (y := ⟨f ⟨x, hx'⟩, by simp [inverse_domain]⟩) (x := ⟨x, hx'⟩) (ker_eq_bot_of_toFun_ker_eq_bot h_ker) rfl

set_option maxRecDepth 4000 in
lemma inverse_compRestricted_eq : f.inverse ∘ᵣ f = domRestrict 1 f.domain := by
  ext x hx hx'
  · rw [mem_compRestricted_domain_iff, inverse_domain, LinearPMap.domRestrict_domain]
    constructor
    · rintro ⟨h, -⟩
      exact ⟨h, trivial⟩
    · rintro ⟨hx, -⟩
      exact ⟨hx, ⟨⟨x, hx⟩, rfl⟩⟩
  · exact inverse_apply_eq (x := ⟨x, hx.2⟩) (ker_eq_bot_of_toFun_ker_eq_bot h_ker) rfl
